...
import { initIcons, createElement } from '../utils.js';
import { triggerHapticImpact } from '../bridge.js';
import { QUIZ_QUESTIONS, getResultByScore } from './quiz-data.js';
import { initQuizDrawer } from './quiz-drawer.js';

// Схема формы дляNotibot
const FORM_SCHEMA = {
...
export function renderQuiz(containerEl) {
  ...
  function renderIntro() {
    containerEl.innerHTML = `
      <div class="card p-6 flex flex-col items-center text-center fade-in">
        ...
        <p class="text-xs text-[var(--color-muted)] leading-relaxed mb-6">
          Быстрый тест из 5 вопросов разработан профессиональным психологом, чтобы оценить ваше текущее состояние и предотвратить синдром эмоционального истощения.
        </p>
  ...
  function renderResult() {
    ...
    const benefitsHtml = BENEFITS.map((b, idx) => `
      ...
      ${idx < BENEFITS.length - 1 ? '<div class="h-px bg-[var(--color-border)] my-4.5 border-dashed"></div>' : ''}
    `).join('');
    ...
    const drawer = initQuizDrawer(FORM_SCHEMA, () => {
      // После успешной отправки можно сбросить квиз
      step = 0;
      totalScore = 0;
      render();
    });
