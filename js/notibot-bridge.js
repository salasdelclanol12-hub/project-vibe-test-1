/** Notibot Bridge v2.26 SDK */
(function () {
    class NotibotBridgeError extends Error {
        constructor(errorData) {
            if (typeof errorData === 'string') {
                super(errorData);
                this.code = 'ERR_UNKNOWN'; this.origin = 'unknown'; this.details = null;
            } else {
    ...
    class NotibotBridge {
        constructor() {
            this.user = {}; this.app = {}; this._updateHandlers = []; this._lastActionTimes = {}; this._responseHandlers = {};
            this._init();
        }
        _init() {
            window.addEventListener('message', (event) => {
                if (event.data?.type === 'NOTIBOT_INIT' && event.data.data) {
                    this.user = event.data.data.user || {}; this.app = event.data.data.app || {};
                    this._updateHandlers.forEach(cb => { try { cb(this.user, this.app); } catch (e) { console.error(e); } });
            ...
            if (typeof ResizeObserver !== 'undefined') {
                const resizeObserver = new ResizeObserver(entries => {
                    for (let entry of entries) {
                        ...
                        if (height > 0) this._sendAction('set_height', { height: height + 60 });
                    }
                });
        ...
        openPortal(config = {}) { this._sendAction('open_portal', config); }
        setScrollLock(locked) { this._sendAction('set_scroll_lock', { locked }); }
        onUpdate(callback) {
        ...
        hapticImpact(style = 'light', fallback = false) { this._sendAction('haptic_feedback', { feedbackType: 'impact', style, fallback }); }
        hapticNotification(type = 'success', fallback = false) { this._sendAction('haptic_feedback', { feedbackType: 'notification', style: type, fallback }); }
        hapticSelection(fallback = false) { this._sendAction('haptic_feedback', { feedbackType: 'selection', fallback }); }
        submitForm(formId, answers) {
            return new Promise((resolve, reject) => {
                ...
                this._responseHandlers[requestId] = (response) => {
                    if (response.success) { resolve(response.data); } else { reject(new NotibotBridgeError(response.error)); }
                };
                if (window.parent) {
                    window.parent.postMessage({ source: 'vibe-sandbox', type: 'submit_form', payload: { formId, answers, requestId } }, '*');
                } else {
                    delete this._responseHandlers[requestId];
        ...
        _sendAction(type, payload = {}) {
            ...
            if (window.parent) window.parent.postMessage({ source: 'vibe-sandbox', type, payload }, '*');
        }
    }
